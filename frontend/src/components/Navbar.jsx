  ...
  return (
    <nav className="glass fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-4 h-20">
      ...
      <div className="hidden md:flex items-center gap-8 text-cream-dark font-sans text-sm tracking-widest uppercase">
        <button onClick={() => scrollToSection('services')} className="hover:text-gold transition-colors duration-300">
          Services
        </button>
        <button onClick={() => scrollToSection('transformation')} className="hover:text-gold transition-colors duration-300">
          Transformations
        </button>
        <button onClick={() => scrollToSection('about')} className="hover:text-gold transition-colors duration-300">
          Our Salon
        </button>
      </div>
      ...
      <div className="hidden md:flex items-center gap-3 relative">
        ...
        {/* Desktop Share Popover — Copy Link only */}
        {showShareMenu && (
          <div className="absolute right-0 top-14 w-52 bg-charcoal-dark/95 border border-gold/20 rounded-xl p-3 shadow-2xl backdrop-blur-lg flex flex-col gap-2 z-50 animate-fade-in text-left">
            ...
            <p className="text-[10px] text-cream-muted font-sans font-light px-3 leading-relaxed">
              Share the link on Instagram or any platform.
            </p>
          </div>
        )}
      </div>
      ...
      {isOpen && (
        <div className="absolute top-20 left-0 right-0 bg-charcoal-dark/95 border-b border-gold/20 backdrop-blur-lg flex flex-col items-center py-8 gap-6 z-40 animate-fade-in shadow-2xl md:hidden">
          <button onClick={() => scrollToSection('services')} className="text-cream hover:text-gold font-sans text-sm tracking-widest uppercase transition-colors">
            Services
          </button>
          <button onClick={() => scrollToSection('transformation')} className="text-cream hover:text-gold font-sans text-sm tracking-widest uppercase transition-colors">
            Transformations
          </button>
          <button onClick={() => scrollToSection('about')} className="text-cream hover:text-gold font-sans text-sm tracking-widest uppercase transition-colors">
            Our Salon
          </button>
          <div className="flex gap-3 mt-2 w-full px-8 justify-center">
            <MagneticButton onClick={handleMobileContact} className="py-2.5 px-6 text-xs flex-1 max-w-[160px]">
              Contact Us
            </MagneticButton>
            <button
              onClick={handleShare}
